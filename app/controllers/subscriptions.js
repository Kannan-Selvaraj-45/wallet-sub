...
export default class SubscriptionsController extends Controller {
  ...
  get anyActiveSubscriptions() {
    if (this.subscriptions.userSubscriptions.length) {
      const active = this.subscriptions.userSubscriptions.some((item) => item.isActive)
      return active
    }
  }
  ...
  get inactiveSubscriptions() {
    const anyInactive = this.subscriptions.userSubscriptions.filter((item) => !item.isActive)
    return anyInactive
  }
  ...
  autoFillAmount() {
    ...
    if (title) {
      const matchedService = this.subscriptions.subsPlans.find(
        (s) => s.service.split(" ")[0].toLowerCase() === title.split(" ")[0].toLowerCase(),
      )

      if (matchedService) {
        const matchedPlan = matchedService.plans.find((p) => p.planName === plan)
  ...
  @action
  saveSubscription() {
    if (!this.newSubscription.title || !this.newSubscription.planPrice ||!this.newSubscription.paymentMethod) {
      this.showAddSubscriptionModal = false
      this.payType=''
    ...
    if (this.isEditing) {
      ...
      const updateSubscriptions = this.subscriptions.userSubscriptions.map((sub) => {
      ...
      if (!findSubscriptionExists) {
        ...
        this.subscriptions.userSubscriptions = [...this.subscriptions.userSubscriptions, newSub]

        this.history.transactions = [...this.history.transactions, newTransaction]

        if (newSub.discount) {
          this.subscriptions.totalDiscounts = this.subscriptions.totalDiscounts + newSub.discount
        }
        ...
        this.flashMessages.warning(`${this.newSubscription.title} subscription already exists!`)
      }
    }
  ...
  subscribe(serviceTitle, serviceCycle) {
    const plan = this.subscriptions.popularPlans.find((p) => p.title === serviceTitle && p.planCycle === serviceCycle)
    if (plan.isOffer) {
      this.newSubscription = {
        title: plan.title,
        plan: plan.plan,
  ...
  deleteSubscription(subscription) {
    this.subscriptions.userSubscriptions = this.subscriptions.userSubscriptions.filter(
    ...
    if (subscription.discount) {
      this.subscriptions.totalDiscounts = this.subscriptions.totalDiscounts - subscription.discount
